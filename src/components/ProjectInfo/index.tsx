...
export function ProjectInfo(
    {
        projectName, projectUrl, projectStatuses
    }: ProjectInfoParms
) {
        ...
        <div className="flex gap-5 flex-wrap">
            {Object.keys(projectStatuses).map((statusKey) => {
                return <div className="flex gap-2 items-center">
                    <p className="text-sm text-text-secondary text-nowrap">{statusKey}</p>
                    <p className="text-nowrap">{projectStatuses[statusKey]}</p>
                </div>
            })}
        </div>
    </div>
